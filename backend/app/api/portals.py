...
from app.models.domain import Portal
...
async def test_portal_auth(portal_id: str, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Portal).where(Portal.id == portal_id))
    portal = result.scalars().first()
    if not portal:
        raise HTTPException(status_code=404, detail="Portal not found")

    score, vulns, tests, report = await scan_portal_security(portal.base_url)
    return {
        "portal_id": portal.id,
